...
parser.add_argument('--label_type', default='glucose', type=str)
...
out_len = 1
model = Model.ClassNet(input_size=feat_len, cls_num=cls_num).to(device)
...
for epoch in range(epochs):
    ...
    for i, data in enumerate(trn_loader):
        ...
        img_seq_base, label_seq = img_seq.to(device), label_seq.to(device)
        ...
        output_vec = torch.reshape(output_vec, (-1,output_vec.size()[-1]))
        label_seq_vec = torch.reshape(label_seq_vec, (-1,))
    ...
    for i, data in enumerate(tst_loader):
        ...
        img_val_base, label_val = img_val.to(device), label_val.to(device)

        img_seq2 = copy.deepcopy(img_val_base)[:,1:]
        img_seq1 = copy.deepcopy(img_val_base)[:,:-1]
        img_val = torch.cat((img_seq1, img_seq2), dim=1)
        ...
        with torch.no_grad():
            ...
            output_vec = torch.reshape(output_vec, (-1,output_vec.size()[-1]))
            label_val_vec = torch.reshape(label_val_vec, (-1,))
