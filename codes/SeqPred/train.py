...
trn_data = lonData('./data', 'train', img_size=224, label_type=args.label_type)
trn_loader = DataLoader(trn_data, batch_size=batch_size, shuffle=True , num_workers=nworkers)
tst_data = lonData('./data', 'test', img_size=224, label_type=args.label_type)
...
if flag_net == 'self':
    ...
    net_fea.to(device)
    num_fits = 2048
    net = Model.downFeaNet(num_fits=2048, feature_size=feature_size)
    pre_epoch = 10
...
if args.feature_merge == True:
    model = TransImgPred(seq_len=img_len, feature_size=feature_size, cls_num=cls_num, nhid=512,
                         nlayers=nlayers, nhead=nlayers, dropout=dropout, merge=True).to(device)
    out_len = 1
else:
    model = TransImgPred(seq_len=img_len, feature_size=feature_size, cls_num=cls_num, nhid=512,
                         nlayers=nlayers, nhead=nhead, dropout=dropout).to(device)
    out_len = 2
...
for epoch in range(epochs):
    ...
    for i, data in enumerate(trn_loader):
        ...
        label_seq_vec = label_seq.permute(1,0).reshape((label_seq.size(0)*img_len))
        if args.feature_merge == True:
            label_seq_vec = label_seq_vec[-label_seq.size(0):]

        # label = {0, 1, 2}; 0- normal, 1- slightly, 2- seriously

        base_size, img_size = base_seq.size(), img_seq.size()
        # if args.feature_merge == True:
        #     tgt_mask = torch.zeros((img_len,img_len)).to(device)
        # elif args.feature_merge == False:
        ...
        if args.feat_net == 'self':
            if epoch < pre_epoch :
                with torch.no_grad():
                    base_fea = net_fea(base_seq)
                    img_fea = net_fea(img_seq)
            else:
                base_fea = net_fea(base_seq)
                img_fea = net_fea(img_seq)
            base_fea = net(base_fea)
            img_fea = net(img_fea)
        else:
            base_fea = net(base_seq)
            img_fea = net(img_seq)

        # end2 = time.time()

        # change to shape [seq_len, bz, fea_len]
        base_fea = base_fea.reshape(*base_size[:2],-1).permute(1,0,2)
        img_fea = img_fea.reshape(*img_size[:2],-1).permute(1,0,2)


        optimizer.zero_grad()
        output, _ = model(base_fea, img_fea, tgt_mask=tgt_mask)
    ...
    for i, data in enumerate(tst_loader):
        ...
        if args.feature_merge or args.single_pred == True:
        ...
        with torch.no_grad():
            ...
            if args.feat_net == 'self':
                base_fea_val = net_fea(base_val)
                img_fea_val = net_fea(img_val)

                base_fea_val = net(base_fea_val)
