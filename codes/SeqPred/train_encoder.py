...
trn_data = dataset.lonDataEn('./data', 'train', img_size=224, label_type=args.label_type)
trn_loader = DataLoader(trn_data, batch_size=batch_size, shuffle=True , num_workers=nworkers)
tst_data = dataset.lonDataEn('./data', 'test', img_size=224, label_type=args.label_type)
...
if args.feature_merge == True:
    model = TransImgPredEn(seq_len=img_len, feature_size=feat_len, cls_num=cls_num, nhid=512,
                         nlayers=nlayers, nhead=nhead, dropout=dropout, merge=True).to(device)
    out_len = 1
else:
    model = TransImgPredEn(seq_len=img_len, feature_size=feat_len, cls_num=cls_num, nhid=512,
...
for epoch in range(epochs):
    ...
    for i, data in enumerate(trn_loader):
        ...
        src_mask = model.generate_square_subsequent_mask(img_size[1]).to(device)
        ...
        optimizer.zero_grad()
        output = model(img_fea, src_mask=src_mask)
    ...
    for i, data in enumerate(tst_loader):
        ...
        with torch.no_grad():
            ...
            output = model(img_fea_val, src_mask=src_mask)
            output_vec = output.reshape((img_size[0]*out_len, -1))
